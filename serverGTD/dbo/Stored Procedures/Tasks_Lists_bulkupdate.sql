CREATE PROCEDURE [Tasks_Lists_bulkupdate]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [Tasks_Lists_BulkType] READONLY
AS
BEGIN
...
MERGE [Tasks_Lists] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Tasks_Lists_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [Task_id] = changes.[Task_id], [List_id] = changes.[List_id], [Owner] = changes.[Owner]
...
FROM 
[Tasks_Lists_tracking] side JOIN 
