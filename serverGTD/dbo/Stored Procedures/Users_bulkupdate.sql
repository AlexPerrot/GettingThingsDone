CREATE PROCEDURE [Users_bulkupdate]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [Users_BulkType] READONLY
AS
BEGIN
...
MERGE [Users] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Users_tracking] t ON p.[Id] = t.[Id]) as changes ON changes.[Id] = base.[Id]
...
UPDATE SET [Username] = changes.[Username], [Password] = changes.[Password], [Mail] = changes.[Mail]
