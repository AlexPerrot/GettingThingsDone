CREATE PROCEDURE [Projects_Tasks_bulkupdate]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [Projects_Tasks_BulkType] READONLY
AS
BEGIN
...
MERGE [Projects_Tasks] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Projects_Tasks_tracking] t ON p.[Id] = t.[Id] AND p.[Owner] = t.[Owner]) as changes ON changes.[Id] = base.[Id] AND changes.[Owner] = base.[Owner]
...
UPDATE SET [Project_id] = changes.[Project_id], [Task_id] = changes.[Task_id]
...
FROM 
[Projects_Tasks_tracking] side JOIN 
