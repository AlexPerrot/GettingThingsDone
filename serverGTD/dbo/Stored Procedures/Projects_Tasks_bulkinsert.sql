CREATE PROCEDURE [Projects_Tasks_bulkinsert]
...
(SELECT p.*, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Projects_Tasks_tracking] t ON p.[Id] = t.[Id] AND p.[Owner] = t.[Owner]) AS changes ON changes.[Id] = base.[Id] AND changes.[Owner] = base.[Owner]
