CREATE PROCEDURE [Users_bulkinsert]
...
(SELECT p.*, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Users_tracking] t ON p.[Id] = t.[Id]) AS changes ON changes.[Id] = base.[Id]
WHEN NOT MATCHED BY TARGET AND changes.local_update_peer_timestamp <= @sync_min_timestamp OR changes.local_update_peer_timestamp IS NULL THEN
INSERT ([Id], [Username], [Password], [Mail]) VALUES (changes.[Id], changes.[Username], changes.[Password], changes.[Mail])
