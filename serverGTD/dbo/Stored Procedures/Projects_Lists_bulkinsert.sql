...
(SELECT p.*, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Projects_Lists_tracking] t ON p.[Id] = t.[Id] AND p.[Owner] = t.[Owner]) AS changes ON changes.[Id] = base.[Id] AND changes.[Owner] = base.[Owner]
WHEN NOT MATCHED BY TARGET AND changes.local_update_peer_timestamp <= @sync_min_timestamp OR changes.local_update_peer_timestamp IS NULL THEN
INSERT ([Id], [Project_id], [Owner], [Title], [Description]) VALUES (changes.[Id], changes.[Project_id], changes.[Owner], changes.[Title], changes.[Description])
