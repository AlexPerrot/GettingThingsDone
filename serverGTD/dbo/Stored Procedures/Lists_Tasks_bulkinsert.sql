CREATE PROCEDURE [Lists_Tasks_bulkinsert]
...
SET IDENTITY_INSERT [Lists_Tasks] ON;
-- update/insert into the base table
MERGE [Lists_Tasks] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Lists_Tasks_tracking] t ON p.[Id] = t.[Id] AND p.[Owner] = t.[Owner]) AS changes ON changes.[Id] = base.[Id] AND changes.[Owner] = base.[Owner]
WHEN NOT MATCHED BY TARGET AND changes.local_update_peer_timestamp <= @sync_min_timestamp OR changes.local_update_peer_timestamp IS NULL THEN
INSERT ([Id], [List_id], [Task_id], [Owner]) VALUES (changes.[Id], changes.[List_id], changes.[Task_id], changes.[Owner])
OUTPUT INSERTED.[Id], INSERTED.[Owner] INTO @changed; -- populates the temp table with successful PKs

SET IDENTITY_INSERT [Lists_Tasks] OFF;
UPDATE side SET
