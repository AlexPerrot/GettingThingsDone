CREATE PROCEDURE [Projects_Lists_Tasks_bulkupdate]
	@sync_min_timestamp BigInt,
	@sync_scope_local_id Int,
	@changeTable [Projects_Lists_Tasks_BulkType] READONLY
AS
BEGIN
...
SET IDENTITY_INSERT [Projects_Lists_Tasks] ON;
-- update the base table
MERGE [Projects_Lists_Tasks] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Projects_Lists_Tasks_tracking] t ON p.[Id] = t.[Id] AND p.[Owner] = t.[Owner]) as changes ON changes.[Id] = base.[Id] AND changes.[Owner] = base.[Owner]
...
UPDATE SET [Project_List_id] = changes.[Project_List_id], [Task_id] = changes.[Task_id]
OUTPUT INSERTED.[Id], INSERTED.[Owner] into @changed; -- populates the temp table with successful PKs

SET IDENTITY_INSERT [Projects_Lists_Tasks] OFF;
UPDATE side SET
...
FROM 
[Projects_Lists_Tasks_tracking] side JOIN 
