CREATE PROCEDURE [Projects_Lists_bulkupdate]
...
SET IDENTITY_INSERT [Projects_Lists] ON;
-- update the base table
MERGE [Projects_Lists] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Projects_Lists_tracking] t ON p.[Id] = t.[Id] AND p.[Owner] = t.[Owner]) as changes ON changes.[Id] = base.[Id] AND changes.[Owner] = base.[Owner]
...
UPDATE SET [Project_id] = changes.[Project_id], [Title] = changes.[Title], [Description] = changes.[Description]
OUTPUT INSERTED.[Id], INSERTED.[Owner] into @changed; -- populates the temp table with successful PKs

SET IDENTITY_INSERT [Projects_Lists] OFF;
UPDATE side SET
