CREATE PROCEDURE [Tasks_Tasks_bulkupdate]
...
SET IDENTITY_INSERT [Tasks_Tasks] ON;
-- update the base table
MERGE [Tasks_Tasks] AS base USING
-- join done here against the side table to get the local timestamp for concurrency check
(SELECT p.*, t.update_scope_local_id, t.scope_update_peer_key, t.local_update_peer_timestamp FROM @changeTable p LEFT JOIN [Tasks_Tasks_tracking] t ON p.[Id] = t.[Id] AND p.[Owner] = t.[Owner]) as changes ON changes.[Id] = base.[Id] AND changes.[Owner] = base.[Owner]
...
UPDATE SET [Predecessor_Id] = changes.[Predecessor_Id], [Successor_Id] = changes.[Successor_Id]
OUTPUT INSERTED.[Id], INSERTED.[Owner] into @changed; -- populates the temp table with successful PKs

SET IDENTITY_INSERT [Tasks_Tasks] OFF;
UPDATE side SET
